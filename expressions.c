...

{
  ...
  for ( a = gactions; a; a = a->next ) {
	...
	if ( lC[j] == 0 ) continue;
	if ( lC[j] > 0 ) {
	  lnf->pF[lnf->num_pF] = lF2[j];
	  lnf->pC[lnf->num_pF++] = lC[j];
	} else {
	  lnf->nF[lnf->num_nF] = lF2[j];
	...
	for ( k = 0; k < lnum_F2; k++ ) {
	  if ( lC[k] == 0 ) continue;
	  if ( lC[k] > 0 ) {
	    lnf->pF[lnf->num_pF] = lF2[k];
	    lnf->pC[lnf->num_pF++] = lC[k];
	  } else {
	    lnf->nF[lnf->num_nF] = lF2[k];
...

{
  ...
  case FHEAD:
    ...
    if ( i < lnum_F2 ) {
      ...
      if ( lnum_F2 == MAX_LNF_F ) {
	printf("\n\nincrease MAX_LNF_F! currently %d\n\n", MAX_LNF_F);
	exit( 1 );
      }
      lF2[lnum_F2] = n->fl;
      lC[lnum_F2] = positive ? n->c : ((-1) * n->c);
      lnum_F2++;
    }
    break;
  default:
...

{
  ...
  while ( TRUE ) {
     ...
     * lF2...; (local global borrowed from above);
     *
...

{
  ...
  for ( i = 0; i < gnum_lnf_goal; i++ ) {
	...
	lF2[j] = glnf_goal_lh[i]->pF[j];
	lC[j] = glnf_goal_lh[i]->pC[j];
      }
      lnum_F2 = glnf_goal_lh[i]->num_pF;
      return TRUE;
    }
  }
  ...
  for ( a = gactions; a; a = a->next ) {
	...
	for ( j = 0; j < a->lnf_preconds_lh[i]->num_pF; j++ ) {
	  lF2[j] = a->lnf_preconds_lh[i]->pF[j];
	  lC[j] = a->lnf_preconds_lh[i]->pC[j];
	}
	lnum_F2 = a->lnf_preconds_lh[i]->num_pF;
	return TRUE;
      }
    }
	...
	if ( e->lnf_conditions_lh[j]->num_pF > 1 ||
	  ...
	  for ( k = 0; k < e->lnf_conditions_lh[j]->num_pF; k++ ) {
	    lF2[k] = e->lnf_conditions_lh[j]->pF[k];
	    lC[k] = e->lnf_conditions_lh[j]->pC[k];
	  }
	  lnum_F2 = e->lnf_conditions_lh[j]->num_pF;
	  return TRUE;
	}
      }
	...
	if ( e->lnf_effects_rh[j]->num_pF > 1 ||
	  ...
	  for ( k = 0; k < e->lnf_effects_rh[j]->num_pF; k++ ) {
	    lF2[k] = e->lnf_effects_rh[j]->pF[k];
	    lC[k] = e->lnf_effects_rh[j]->pC[k];
	  }
	  lnum_F2 = e->lnf_effects_rh[j]->num_pF;
	  return TRUE;
	}
      }
    }
...

{
  ...
  for ( i = 0; i < lnum_F2; i++ ) {
    grelevant_fluents_lnf[gnum_relevant_fluents]->pF[i] = lF2[i];
    grelevant_fluents_lnf[gnum_relevant_fluents]->pC[i] = lC[i];
  }
  grelevant_fluents_lnf[gnum_relevant_fluents]->num_pF = lnum_F2;
